...
class LatentRE(nn.Module):
    ...
    def forward(self, 
        ...
        if Config.training:
            ...
            pos_score = torch.mean(torch.sum(pos_logit * pos_label, 1)) #should be 1

            Config.train_bag = False
            neg_text = self.encoder(neg_word, neg_pos1, neg_pos2)
            neg_logit, _ = self.selector(neg_text, None, None)
            neg_score = torch.mean(torch.sum(neg_logit * mul_label, 1)) # should be 0
            neg_pos_score = torch.mean(torch.sum(neg_logit * neg_label, 1)) # should be 1
