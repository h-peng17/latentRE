...
class LatentRE(nn.Module):
    def __init__(self, word_vec, weight=None):
        ...
        ''' load encoder '''
        # checkpoint = torch.load(os.path.join(Config.save_path, "ckptbert1"))
        self.encoder = Bert()
    ...
    def forward(self, 
                  ...
                  scope=None):
        if Config.training:
            main_text = self.main_encoder(input_ids, attention_mask)
            ...
            if Config.latent:
                ...
                return bce_loss * Config.kl_loss_scale + ce_loss * Config.ce_loss_scale
        else:
